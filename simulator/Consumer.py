...
class Consumer(object):
    ...
    def open_channel(self):
        ...
        obj = self._connection.channel(on_open_callback=self.on_channel_open)
    ...
    def run_2(self, callback):
        """Run the example consumer by connecting to RabbitMQ and then
        starting the IOLoop to block and allow the SelectConnection to operate.

        """
        self._message_callback = callback
        self._connection = self.connect_2()
        self._connection.add_timeout(1, self.on_timeout)
        self._connection.ioloop.start()
...
def main():
    ...
    example = Consumer('amqp://Fm:Fm@162.243.66.171:5672/%2Fbunny')
    try:
        example.run()
